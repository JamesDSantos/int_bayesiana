...
Seja $S=\{1,\ldots,m\}$ o conjunto das $m$ localidades. Suponha que é razoável particionar $S$ e $k$ grupos. Por último, considere que todas as localidades dentro de uma partição compartilham a mesma taxa, ou seja, para $j$ dentro da partição $S_t$, 
...
$$\prod_{j=1}^k \lambda_j^{z_{i,j}}=\lambda_t=\sum_{j=1}^k \lambda_j z_{i,j}.$$
Deste modo, temos que $y_i|z_i,\lambda\sim\hbox{Poisson}(\prod_{j=1}^k\lambda_j^{z_{i,j}} N_i)$. e a função de verossimilhança é dada por

$$L(\lambda,z)=\prod_{i=1}^m \frac{e^{-\sum_{j=1}^k\lambda_jz_{i,j}N_i}[\prod_{j=1}^k \lambda_j^{z_{i,j}} N_i]^{y_i}}{y_i!}\propto \prod_{i=1}^m \left[\prod_{j=1}^ke^{-\lambda_jz_{i,j}N_i}\right]\left[\prod_{j=1}^k \lambda_j^{z_{i,j}} N_i\right]^{y_i}=\prod_{j=1}^k e^{-\lambda_j\sum_{i=1}^m z_{i,j}N_i}\lambda_j^{\sum_{i=1}^m z_{i,j}y_i}$$
Observe que $z_j$ é desconhecido e vamos tratá-lo como parâmetro. Pela sua natureza, pode-se pensar em utilizar a priori $z_j\sim\hbox{Bernoulli Multivariada}(p_1,\ldots,p_k)$.
...
$$\begin{align}f(\lambda,z|\boldsymbol{y})&\propto L(\lambda,z)f(\lambda)f(z)\\&\propto\prod_{j=1}^k e^{-\lambda_j\sum_{i=1}^m z_{i,j}N_i}\lambda_j^{\sum_{i=1}^m z_{i,j}y_i}\prod_{j=1}^k \lambda_j^{r_j-1}e^{-\lambda_js_j}\prod_{i=1}^m \prod_{j=1}^k p_j^{z_{i,j}}\end{align}$$
Note que
$$f(\lambda|z,\boldsymbol{y})$$
